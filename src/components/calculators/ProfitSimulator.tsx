    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 animate-fade-up">
      <div className="bg-surface border border-rim rounded-r-xl p-7">
        ...
        <div className="space-y-5">
          <div>
            <label className="block text-[11.5px] font-semibold tracking-wider uppercase text-t-2 mb-2">Custo total (R$)</label>
              ...
              <div className="relative inline-block">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-end pb-2">
                  <div className="font-montserrat font-extrabold text-5xl tracking-tighter" style={{ color }}>
                    {mg}<sup className="text-xl text-t-2 align-super">%</sup>
                  </div>
                  <div className="text-[11px] font-semibold tracking-widest uppercase text-t-3 mt-1">Margem</div>
                </div>
              </div>
            </div>
            <div className="flex justify-between items-center py-3.5 border-b border-rim">
              <span className="text-[13.5px] text-t-2">Custo total</span>
              <span className="text-[15px] font-semibold">R$ {fmt(custo)}</span>
            </div>
            <div className="flex justify-between items-center py-3.5 border-b border-rim">
              <span className="text-[13.5px] text-t-2">Valor final</span>
              <span className="text-[15px] font-semibold text-amber">R$ {fmt(final)}</span>
            </div>
            <div className="mt-5 bg-ink-tertiary border rounded-r-lg p-5 flex justify-between items-center shadow-amber-glow" style={{ borderColor: color }}>
              <span className="font-montserrat font-bold text-sm uppercase tracking-wider text-t-2">Lucro</span>
              <span className="font-montserrat font-extrabold text-3xl tracking-tighter" style={{ color }}>R$ {fmt(lv)}</span>
            </div>
          </div>
        ) : (
          <p className="text-t-3 text-sm leading-relaxed py-3">Informe o custo e a margem para simular.</p>
        )}
      </div>
    </div>
  );
